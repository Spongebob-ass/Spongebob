...
local UI_URL = "https://raw.githubusercontent.com/Spongebob-ass/Spongebob/refs/heads/main/rift-android-vng.lua"
...
local function loadUI()
    maketoast("Đang tải giao diện người dùng Rift...")
    task.wait(3)
...
-- # Keyless checker
if IS_KEYLESS then
    maketoast("Rift hiện đã được mở khóa, đang tải giao diện người dùng...")
    task.wait(1)
    return loadUI()
end
...
if isfile(KEY_FILE) then
    ...
    or KeyGuardLibrary.validatePremiumKey(savedKey) == TRUE_DATA then
        maketoast("Đã lưu khóa xác thực, đang tải UI...")
        return loadUI()
    end
end
...
local function createKeyUI()
    ...
    keyTextbox.PlaceholderText = "Nhập chìa khóa của bạn..."
    keyTextbox.Text = ""
    ...
    validateBtn.MouseButton1Click:Connect(function()
        ...
        if isValidDefault or isValidPremium then
            ...
            gui:Destroy()
            maketoast(isValidPremium and "Đã xác minh khóa cao cấp!" or "Khóa đã được xác thực!")
            loadUI()
        else
            maketoast("Khóa không hợp lệ, vui lòng thử lại.")
    ...
    joinBtn.Text = "Tham gia Roblox thường xuyên"
    joinBtn.Font = Enum.Font.SourceSans
    joinBtn.TextColor3 = Color3.fromRGB(194, 194, 194)
    joinBtn.TextSize = 14
    joinBtn.MouseButton1Click:Connect(function()
        maketoast("Quay lại trò chơi để hiển thị lại hệ thống chính.")
        gui:Destroy()
    end)
end
