...
local function WLJM_script() -- CheatBox.LocalScript 
	...
	textBox.Text = '-- Đặt kịch bản của bạn ở đây!'
	updateHighlighting()
end
...
local function ORLY_script() -- ExecuteButton.LocalScript 
	...
	btn.MouseButton1Click:Connect(function()
		...
		if code == "" or code:match("^%s*$") then
			maketoast("Vui lòng nhập tập lệnh! Hộp văn bản không được để trống.")
			return
		end
		...
		if not success then
			maketoast("Không thực thi được tập lệnh:\n" .. tostring(result))
		else
			maketoast("Kịch bản đã được thực thi thành công!")
		end
	end)
end
